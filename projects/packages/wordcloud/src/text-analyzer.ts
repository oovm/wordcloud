...
export class TextAnalyzer extends DataAnalyzer {
    analyzeText(text: string, options: AnalyzeOptions = {}): WordFrequency[] {
        ...
        const tokens = this.tokenize(normalized, options.language ?? "auto", options.includeNumbers ?? false);
    ...
    private tokenize(text: string, language: AnalyzeOptions["language"], includeNumbers: boolean): string[] {
        if (language === "chinese") {
