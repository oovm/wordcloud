...
#[test]
fn main() {
    let mut wordcloud = WordCloudCanvas::default().with_word_margin(10).with_rng_seed(1);

    let mask_buf = include_bytes!("stormtrooper_mask.png");
    let mask_image =
        image::load_from_memory_with_format(mask_buf, ImageFormat::Png).expect("Unable to load mask from memory").to_luma8();
    ...
    wordcloud_image.save("a_new_hope.png").expect("Unable to save image a_new_hope.png");
}
